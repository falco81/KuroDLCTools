...
var
  H: PArcHandle; E: TP3AEntry; DirPath: AnsiString;
...
var
  H: PArcHandle; E: TP3AEntry; HD: PHeaderDataA; DirPath: AnsiString;
  RealCount, VirtIdx: Integer;
begin
  ...
  begin
    ...
    if E.CmpSize > MaxInt then HD^.PackSize := MaxInt else HD^.PackSize := LongInt(E.CmpSize);
    if E.UncSize > MaxInt then HD^.UnpSize := MaxInt else HD^.UnpSize := LongInt(E.UncSize);
    HD^.Method   := Integer(E.CmpType);
...
var
  H: PArcHandle; E: TP3AEntry; Buf: PByte; Full: AnsiString; F: TFileStream;
  Aborted: Boolean;
begin
  H := PArcHandle(hArcData);
  ...
  try
    ...
    if Assigned(GProcessDataProcW) then
      Aborted := (GProcessDataProcW(nil, LongInt(E.UncSize and $FFFFFFFF)) = 0)
    else if Assigned(GProcessDataProc) then
      Aborted := (GProcessDataProc(nil, LongInt(E.UncSize and $FFFFFFFF)) = 0);
...
begin
  ...
  try
    ...
    for i := 0 to 7 do if Magic[i] <> Want[i] then Exit;
    Result := True;
  finally CloseFile(F); end;
end;
...
// Build a TP3AWriter populated with all entries from the existing archive
...
function CarryOverExisting(const ArcPath: AnsiString;
                           ...
                           ArchiveExists: Boolean): TP3AWriter;
var
  Arc: TP3AArchive;
  i: Integer;
  CmpBuf: TBytes;
  ...
  EntryWin: AnsiString;
begin
  Result := TP3AWriter.Create;
  if not ArchiveExists then Exit;

  try
    Arc := TP3AArchive.Create(ArcPath);
  except
  ...
  try
    ...
    begin
      ...
      // don't leak to external extractors (Python tools, the game
      // itself, etc).
      ...
      if not Arc.ReadCompressedBytes(i, @CmpBuf[0]) then Continue;

      Result.AddFromCompressed(E.Name, E.CmpType, CmpBuf, E.UncSize,
...
function FinalizeWrite(W: TP3AWriter; const ArcPath: AnsiString): Integer;
var
  TempPath: AnsiString;
begin

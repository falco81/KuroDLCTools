...
| `shops_find_unique_item_id_from_kurodlc.py` | Extracts unique `item_id` from `.kurodlc.json` file (from costume and shops definitions)|
| `shops_create.py` | Generates shop definitions from a template JSON file |
...
| `find_unique_item_id_from_kurodlc.py` | Extracts unique `item_id` from `.kurodlc.json` (only from costume not from shops) file, or from all `.kurodlc.json` in directory |
...
```bash
python shops_find_unique_item_id_from_kurodlc.py path/to/file.kurodlc.json
```
...
- item_ids from shops_find_unique_item_id_from_kurodlc.py output
...
```bash
python find_unique_item_id_from_kurodlc.py path/to/file.kurodlc.json
```
